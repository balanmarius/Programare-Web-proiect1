...
while True:
    ...
    if elementeLineDeStart[0] == 'POST':
        with open("C:/Users/Marius/proiect1-balanmarius/continut/resurse/utilizatori.json", "a") as myFile:
            first = cerere.find('{')
            second = cerere.find('}')
            if (second > -1 and text == ''):
                text = cerere[first:second+1]
                myFile.write(',\n' + text)
    fisier = None
    try:
        ...
        clientsocket.sendall(('Content-Length: ' +
                              str(os.stat(numeFisier).st_size) + '\r\n').encode('UTF-8'))
